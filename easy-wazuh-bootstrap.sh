...
EASY_WAZUH_RECOMMENDED_DISK_FREE_GB="${EASY_WAZUH_RECOMMENDED_DISK_FREE_GB:-500}"
DEPLOYMENT_TOPOLOGY=""
...
for RESOURCE_SETTING in EASY_WAZUH_MIN_VCPU EASY_WAZUH_RECOMMENDED_VCPU EASY_WAZUH_MIN_MEMORY_GB_SINGLE_NODE EASY_WAZUH_MIN_MEMORY_GB_MULTI_NODE EASY_WAZUH_MIN_DISK_FREE_GB_SINGLE_NODE EASY_WAZUH_MIN_DISK_FREE_GB_MULTI_NODE EASY_WAZUH_RECOMMENDED_DISK_FREE_GB; do
...
check_host_resources() {
  ...
  echo "  Free disk space:  ${DISK_FREE_GB} GB available for $INSTALL_PATH, minimum ${REQUIRED_DISK_FREE_GB} GB, recommended ${EASY_WAZUH_RECOMMENDED_DISK_FREE_GB} GB"
  echo ""
  ...
  elif [ "$DISK_FREE_GB" -lt "$EASY_WAZUH_RECOMMENDED_DISK_FREE_GB" ]; then
    echo "Warning: free disk ${DISK_FREE_GB} GB is below the recommended ${EASY_WAZUH_RECOMMENDED_DISK_FREE_GB} GB."
    echo "The installer will continue, but indexed alerts can fill the disk quickly."
    echo ""
  fi
