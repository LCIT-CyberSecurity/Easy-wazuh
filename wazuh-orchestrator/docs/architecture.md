...
- `metrics.py` / `wazuh_api.py`: host/container/Wazuh signal collection plus Wazuh API cluster validation behind mockable interfaces.
...
- `backends/compose.py`: Compose command construction, desired-state override, worker-specific config derivation, targeted Docker operations and targeted NGINX updates.
...
The Compose override is desired state, not history. A removed worker must disappear from `generated/docker-compose.orchestrator.yml` so a future Compose operation cannot resurrect it. New workers receive a generated config under `generated/worker-configs/` derived from the baseline worker config; the cluster key is copied but never logged.
...
`INTEGRATION_VALIDATION_REQUIRED`: automatic incremental worker certificate generation remains blocked until validated on a real Easy-Wazuh/Wazuh Docker host. Docker health polling, Wazuh API cluster validation and NGINX validate/reload are implemented through mockable adapters, but still require integration testing before production use.
