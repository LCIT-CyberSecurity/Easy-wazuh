...
The installer performs a preflight resource check before package installation, Docker pulls, certificate generation, or Compose startup. Defaults are 4 vCPU, 8 GB RAM and 50 GB free disk for single-node, and 4 vCPU, 16 GB RAM and 100 GB free disk for multi-node. These are minimum guardrails; the recommended multi-node lab profile above remains 200 GB or more. For exceptional lab troubleshooting only, override with `EASY_WAZUH_SKIP_RESOURCE_CHECK=yes` or adjust `EASY_WAZUH_MIN_VCPU`, `EASY_WAZUH_MIN_MEMORY_GB_SINGLE_NODE`, `EASY_WAZUH_MIN_MEMORY_GB_MULTI_NODE`, `EASY_WAZUH_MIN_MEMORY_GB`, `EASY_WAZUH_MIN_DISK_FREE_GB_SINGLE_NODE`, `EASY_WAZUH_MIN_DISK_FREE_GB_MULTI_NODE`, or `EASY_WAZUH_MIN_DISK_FREE_GB`.
